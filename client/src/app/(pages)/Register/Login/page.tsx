...
export default function LoginModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    if (isOpen) {
      setIsAnimating(true); // Animate in when opening
    } else {
      setTimeout(() => setIsAnimating(false), 300); // Wait for the closing animation to finish before hiding
    }
  }, [isOpen]);

  // Close handler to trigger the transition before closing the modal
  const handleClose = () => {
    setIsAnimating(false);  // Trigger the closing animation
    setTimeout(() => {
      onClose();  // After the animation ends, fully close the modal
    }, 300); // Match the duration of the closing transition
  };
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex justify-center items-center select-none">
          <div
            className={`bg-gradient-to-b from-yellow-300 to-yellow-50 p-8 rounded-lg shadow-lg w-full max-w-md transform transition-transform duration-300 ease-in-out ${
              isAnimating ? 'translate-y-0' : 'translate-y-[-100vh]'
            }`}
            style={{ filter: "drop-shadow(0 0 5px aliceblue)" }}
          >
            <button
              onClick={handleClose}  // Close when clicked
            ...
            <div className={`${isForgotPassword ? 'opacity-0 translate-x-[100%]' : 'opacity-100 translate-x-0'} transition-all duration-300 ease-in-out`}>
              <div className="mb-8 items-center justify-center flex flex-col">
                ...
                <h2 className="text-xl font-semibold text-center text-[#444] mt-2 ml-5">تسجيل <span className="text-orange-500">الدخول</span></h2>
              </div>
